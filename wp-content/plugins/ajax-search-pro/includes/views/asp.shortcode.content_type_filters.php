...
$_checked = array(
    ...
    "comments" => in_array('comments', $_carr) ? ' checked="checked"' : "",
...
<fieldset class="asp_content_type_filters">
            ...
            <div class="asp_option asp_option_cat <?php echo $_extra_class; ?>">
                <div class="asp_option_inner">
                    <input type="checkbox" value="<?php echo $_o_val; ?>" id="set_<?php echo $fe_field.$id; ?>"
                           aria-label="<?php echo asp_icl_t("Content Type field[".$fe_field."]" . " ($real_id)", $_st['content_type_filter']['labels'][$fe_field], true); ?>"
                           <?php echo $_extra_attr; ?>
                           name="asp_ctf[]" <?php echo $_checked[$fe_field]; ?>/>
                    <label aria-hidden="true" for="set_<?php echo $fe_field.$id; ?>"></label>
                </div>
    ...
    <?php else: ?>
        <div class='asp_select_label asp_select_single'>
            <select name="asp_ctf[]" aria-label="<?php echo asp_icl_t("Content Type field[".$fe_field."]" . " ($real_id)", "Content select field for $fe_field", true); ?>">
                <?php foreach ( $_st['content_type_filter']['selected'] as $fe_field ): ?>
                    <?php
                    $_o_val = $fe_field;
                    if ( $fe_field == 'any' ) {
