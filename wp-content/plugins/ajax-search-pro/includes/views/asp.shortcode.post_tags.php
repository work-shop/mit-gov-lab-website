...
?>
<fieldset aria-label="<?php echo asp_icl_t( "Post tags filter header" . " ($real_id)", w_isset_def($style['frontend_tags_header'], "Filter by Tags"), true ); ?>">
        ...
        if ( $style['display_all_tags_check_opt'] == 1) {
            ...
            <div class="asp_option asp_option_cat asp_option_cat_level-0 asp_option_selectall">
                <div class="asp_option_inner">
                    <input id="<?php echo $id; ?>_post_tag_set_all" type="checkbox"
                           aria-label="<?php echo asp_icl_t("Select all text (post_tag) " . " ($real_id)", $style['all_tags_check_opt_text'], true); ?>"
                           data-targetclass="asp_post_tag_checkbox"
                        <?php echo( ( $style['all_tags_check_opt_state'] == "checked" ) ? 'checked="checked"' : '' ); ?>/>
                    <label aria-hidden="true" for="<?php echo $id; ?>_post_tag_set_all"></label>
                </div>
        ...
        <div class="asp_option asp_option_cat">
            <div class="asp_option_inner">
                <input type="checkbox" value="<?php echo $_sftag->term_id; ?>"
                       aria-label="<?php echo esc_attr($_sftag->name); ?>"
                       id="<?php echo $id; ?>post_tag_set_<?php echo $_sftag->term_id; ?>"
                       class="asp_post_tag_checkbox"
                       name="post_tag_set[]" <?php echo $selected; ?>/>
                <label aria-hidden="true" for="<?php echo $id; ?>post_tag_set_<?php echo $_sftag->term_id; ?>"></label>
            </div>
    ...
    <?php elseif (
        ...
        <select name="post_tag_set[]" <?php echo $_sfto['display_mode'] == 'multisearch' ? ' multiple' : ''; ?>
                aria-label="<?php echo asp_icl_t( "Post tags filter header" . " ($real_id)", w_isset_def($style['frontend_tags_header'], "Filter by Tags"), true ); ?>"
